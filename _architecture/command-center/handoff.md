...
- Compact four-button controls appear under every story on the homepage, dated brief, and permanent shared-story page: **Very useful, Useful, Neutral, Not useful**. Shared links retain the Share control and share count but never reveal or transfer the sender's browser-local selection.
...
- Production target: homepage and dated brief each have six rating groups/24 controls; every permanent shared-story page has one fresh rating group/four controls plus a Share control and share count, without sender rating state in its URL; `/feedback/` is a form-free retirement notice.
