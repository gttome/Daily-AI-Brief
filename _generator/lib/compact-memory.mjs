...
import {historicalEditionSource,historicalEditionStories} from './historical.mjs';
import {noveltyMatches} from './novelty.mjs';
import {sha256} from './util.mjs';
const VERSION='compact-memory-v2';
export function compactMemory(repoRoot,endDate,{cacheFile=null,days=30}={}){
 ...
 for(const date of dates){
   const source=historicalEditionSource(repoRoot,date),hash=sha256(source.evidence_ref+'\n'+source.text);
   ...
   if(previous?.source_hash===hash&&Array.isArray(previous.stories)){records=previous.stories;reused++;}
   else{records=historicalEditionStories(source,date).map(s=>({story_id:s.story_id,brief_date:s.brief_date,headline:s.headline,
     ...
     evidence_ref:source.evidence_ref}));parsed++;}
