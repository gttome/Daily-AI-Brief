...
export function retainCandidates(items,{limit=500}={}){
 ...
 const byUrl=new Map();
 // URL date is only a retention hint, never verified publication evidence.
 const dateHint=x=>x.published_at||x.publication_date||(/\/(20\d{2}-\d{2}-\d{2})(?:[-/]|$)/.exec(x.canonical_url||x.url||'')?.[1])||'';
 const score=x=>Number(x.owner_priority==='high')*10+Number(!!dateHint(x))*2+Number(!!x.snippet);
 ...
 const compare=(a,b)=>score(b)-score(a)||String(dateHint(b)).localeCompare(String(dateHint(a)))||a.url.localeCompare(b.url);
 ...
 return {candidates:selected,retention:{policy:'source_round_robin_priority_recency_url_v2',discovered_unique:byUrl.size,retained:selected.length,
