...
export function expandEditorialKernel(kernel,{candidateFacts,imageAssets,metadataCandidates,media,publishedAt,coveragePeriod,createdBy='deterministic-kernel-expander'}={}){
 ...
 const videoSlots=media?.worth_watching||{general:{status:'empty',exception:'No video met today’s editorial quality standards.'},agents_non_technical_people:{status:'empty',exception:'No video met today’s editorial quality standards.'}};
 const publishablePodcasts=Array.isArray(media?.podcasts)?media.podcasts.filter(item=>item?.status==='included'):[];
 if(normalized.brief_date>='2026-09-19'){
  const includedVideos=['general','agents_non_technical_people'].filter(key=>videoSlots?.[key]?.status==='included').length;
  if(includedVideos!==2)throw Error('September 19+ production contract requires exactly two included videos');
  if(publishablePodcasts.length!==2)throw Error('September 19+ production contract requires exactly two included podcasts');
 }
 const base={schema_version:'1.0.0',policy_profile:'under80-v1',edition_id:normalized.edition_id,brief_date:normalized.brief_date,timezone:'America/Chicago',title:`Daily Generative AI Brief — ${new Date(`${normalized.brief_date}T12:00:00Z`).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric',timeZone:'UTC'})}`,published_at:publishedAt,research_cutoff_at:researchCutoffAt,coverage_period:canonicalCoverage,status:'staged',stories,worth_watching:videoSlots,editorial_takeaway:normalized.editorial_takeaway,provenance:{created_by:createdBy,created_at:publishedAt,source_commit:normalized.baseline_sha}};
