...
export function renderSubscriptionCard(){return `<section class="subscription-card" aria-labelledby="subscribe-title"><h2 id="subscribe-title">Follow the daily brief</h2><p>One entry per day in your feed reader. Choose a date to open its complete archived brief.</p><p><a class="rss-follow" href="{{ '/subscribe/' | relative_url }}">Follow the daily brief</a> · <button type="button" class="rss-help-open" hidden>How to subscribe</button></p><p>No email address or account on this site needed.</p><p><a href="{{ '/daily-feed.xml' | relative_url }}">Daily edition RSS feed</a> · <a href="{{ '/feed.xml' | relative_url }}">Individual-item feed</a></p></section>`;}
