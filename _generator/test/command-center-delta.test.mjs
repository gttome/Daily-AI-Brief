...
test('Command Center delta packet is allowlisted and excludes private/arbitrary input fields',()=>{
 const packet=commandCenterDeltaPacket({
  validation:{date:'2026-09-18',publication_sha:'a'.repeat(40),final_result:'pass',model_calls:0,semantic_escalation_required:false,automatic_ai_recovery_runs:0,coverage:{articles:6,videos:0,podcasts:2,included_items:8,potential_positions:10},domain_states:{publication:'verified',coverage:'degraded'},private_reader_data:{ratings:[1,2,3]},checks:[{check_id:'routes',result:'pass',severity:'critical',evidence:'do not copy detailed evidence',owner_email:'private@example.com'}]},
 ...
 assert.equal(packet.validation.coverage.included_items,8);assert.equal(packet.validation.domain_states.coverage,'degraded');assert.equal(packet.validation.automatic_ai_recovery_runs,0);
