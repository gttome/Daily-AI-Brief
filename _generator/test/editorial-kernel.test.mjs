...
test('deterministic expansion owns ids slugs URLs source metadata image metadata and social derivatives',()=>{
 ...
 for(let i=0;i<6;i++){
  candidateFacts[`candidate-${i+1}`]={event_date:'2026-09-17',companies:['Example AI'],source_word_count:800+i,source_word_count_method:'retrieved_source_text_whitespace_v1',source_fetched_at:'2026-09-18T11:00:00Z',source:{title:`Source ${i+1}`,organization:'Example AI',url:`https://example.org/story-${i+1}`,publication_date:'2026-09-17',evidence_type:'official_announcement',availability_status:'general_availability'},novelty:{disposition:'new',prior_story_ids:[],what_changed:null},candidate_score:{significance:4,freshness:4,authority:5,evidence_quality:4,novelty:4,practical_value:4,category_fit:5,total:30},selection_rationale:'Highest qualifying candidate in its approved category.'};
  metadataCandidates.candidates.push({candidate_id:`candidate-${i+1}`,canonical_url:`https://example.org/story-${i+1}`,published_at:i<2?'2026-09-18T10:00:00Z':'2026-09-17T10:00:00Z'});
