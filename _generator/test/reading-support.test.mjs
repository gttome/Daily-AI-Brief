...
test('approved article and podcast context is safe and preserves uncertainty',()=>{
 ...
 const a=renderReadingSupport(edition.stories[0],edition.stories[0].story_id,edition.brief_date);assert.match(a,/Update/);assert.match(a,/Agent ensemble/);
});
...
test('related future items and duplicate selections are rejected',()=>{
 const x={item_id:edition.stories[0].story_id,coverage_label:'Background',label_reason:'reason',context_term:'term',context:'context',related:{title:'future',connection:'test',brief_date:'2026-09-13',url:'https://gttome.github.io/Daily-AI-Brief/stories/2026-09-13/test/'}};
