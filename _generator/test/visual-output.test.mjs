...
test('validated handoff image review accepts six distinct accepted OpenAI PNGs and detects tampering',()=>{
 ...
 const stories=[];
 for(let i=0;i<6;i++){
  const id=`c${i+1}`,rel=`briefs/images/${date}/0${i+1}-story.png`,bytes=png(1200+i,630);
  ...
  manifest[id]={path:rel,alt,width:1200+i,height:630,sha256:hash,quality_accepted:true,generation_method:'openai_image_generation'};
  stories.push({story_id:`story-${i+1}`,image:{path:rel,alt,width:1200+i,height:630}});
 }
