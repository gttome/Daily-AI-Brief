...
for(let i=0;i<monitored.length;i+=5)await Promise.all(monitored.slice(i,i+5).filter(s=>s.status==='active').map(async s=>{
 try{const result=await retrieveSource(s.discovery_endpoint);let found=0;
 for(const link of extractLinks(result.text,result.url)){
 const {title}=link;if(title.length<20||title.length>200||(!s.broad_discovery&&!/agent|AI|context|retrieval|evaluation|copilot|gemini|claude|skill|prompt|LLM/i.test(title))||found>=100)continue;
 const url=new URL(link.url);if(url.search)continue;
 ...
 }sources.push({source_id:s.source_id,status:'retrieved',candidate_links:found,checked_at:new Date().toISOString(),attempts:result.attempts});
