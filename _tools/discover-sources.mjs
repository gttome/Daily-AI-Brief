...
const retrievalCache=new RetrievalCache({directory:process.env.DAB_RETRIEVAL_CACHE||null});
...
for(let i=0;i<monitored.length;i+=5)await Promise.all(monitored.slice(i,i+5).filter(s=>s.status==='active').map(async s=>{
 try{const result=await retrievalCache.retrieve(s.discovery_endpoint,retrieveSource,{kind:'metadata'});let found=0;
