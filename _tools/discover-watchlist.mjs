...
for(let i=0;i<monitored.length;i+=4)await Promise.allSettled(monitored.slice(i,i+4).map(async s=>{
 ...
 try{const result=await retrieveSource(s.endpoint);let found=0;
 ...
 const {title}=link;if(title.length<24||title.length>220)continue;
 const u=new URL(link.url);if(/login|signup|privacy|terms|subscribe|javascript/i.test(u.pathname))continue;
 ...
 checks.push({source_id:s.source_id,status:found?'retrieved':'no_candidate_links',candidate_links:found,attempts:result.attempts,checked_at:new Date().toISOString()});
 }catch(e){checks.push({source_id:s.source_id,status:'unavailable',reason:e.message,attempts:e.attempts||[],checked_at:new Date().toISOString()});}
}));
const coverage={checked:checks.length,retrieval_success:checks.filter(x=>['retrieved','no_candidate_links'].includes(x.status)).length,with_candidate_links:checks.filter(x=>x.status==='retrieved').length,no_candidate_links:checks.filter(x=>x.status==='no_candidate_links').length,retrieval_failures:checks.filter(x=>x.status==='unavailable').length,assisted_review_required:checks.filter(x=>x.status==='assisted_review_required').length};
const data={updated_at:now,coverage,candidates:[...candidates.values()].sort((a,b)=>b.last_seen.localeCompare(a.last_seen)).slice(0,2000),sources:checks.sort((a,b)=>a.source_id.localeCompare(b.source_id)),early_signal_channels:early.channels.length,note:'Discovery leads only. Work research must group by meaning, verify original evidence and dates, and evaluate novel topics before promotion. Retrieval health and candidate yield are distinct: no extracted links is not a transport failure and does not establish no relevant news. Assisted review is reported separately. A failed retrieval is not no news; community and aggregator activity is not proof.'};
