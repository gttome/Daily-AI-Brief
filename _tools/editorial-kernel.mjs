...
if(command==='validate'){
 ...
 for(const required of ['facts','images','published-at','coverage'])if(!args[required])throw Error(`Requires --${required}`);
 const candidateFacts=read(args.facts),imageAssets=read(args.images),media=args.media?read(args.media):null;
 const edition=expandEditorialKernel(kernel,{candidateFacts,imageAssets,media,publishedAt:args['published-at'],coveragePeriod:args.coverage});
