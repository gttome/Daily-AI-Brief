#!/usr/bin/env node
import {RunTelemetry} from '../_generator/lib/run-telemetry.mjs';
...
if(command==='init'){
 ...
 const initialTelemetry=new RunTelemetry(path.join(privateRoot,m.attempt_id,'telemetry.jsonl'),{editionId:'dab-edition-'+m.edition_date,attemptId:m.attempt_id,baselineSha:m.baseline_sha});initialTelemetry.lifecycle('started');
 ...
 const telemetry=new RunTelemetry(path.join(root,'telemetry.jsonl'),{editionId:'dab-edition-'+m.edition_date,attemptId:m.attempt_id,baselineSha:m.baseline_sha});
 const inputs=['_tools/production-run.mjs','_tools/discover-sources.mjs','_tools/discover-watchlist.mjs','_tools/discovery-context.mjs','_tools/discovery-links.mjs','_generator/lib/research.mjs','_generator/lib/incremental-watchlist.mjs','_generator/lib/discovery-queue.mjs','_generator/lib/production-run.mjs','_generator/lib/media-evidence.mjs','_generator/lib/media-selection.mjs','_generator/lib/visual-recovery.mjs','_generator/lib/image-gate.mjs','docs/images/publisher-policy.md','docs/podcasts/publisher-policy.md','docs/operations/efficiency-operating-policy-2026-09-17.md','docs/operations/efficiency-operating-policy.json','_data/command-center-access-policy.json','_generator/lib/compact-memory.mjs','_generator/lib/historical.mjs','_generator/lib/checkpoints.mjs','_generator/lib/util.mjs','_data/watchlist-sources.json','_data/required-topic-sources.json','_data/preflight-source-plan.json','docs/operations/publisher-runbook.md','_data/source-registry.json','_data/early-signal-sources.json','_data/emerging-signal-policy.json'];
 ...
 if(command==='discover'){
  ...
  const result=await checkpoints.run('discovery',{inputs:['inputs/runtime.json'],alwaysRun:expired||['media-candidate-queue','watchlist-discoveries','watchlist-source-state'].some(name=>{const live=path.join(repo,'_data',name+'.json'),saved=path.join(root,'outputs',name+'.json');return fs.existsSync(live)&&fs.existsSync(saved)&&sha256(fs.readFileSync(live))!==sha256(fs.readFileSync(saved));}),execute:async()=>{
   ...
   await telemetry.measure('discovery',()=>import('./discover-sources.mjs'));
   await telemetry.measure('watchlist',()=>import('./discover-watchlist.mjs'));
   ...
   const {acquisition}=await import('./discovery-context.mjs');for(const [key,value] of Object.entries(acquisition.cache.metrics)){if(['hits','misses','cache_hits','cache_misses'].includes(key))telemetry.observe('research.'+(key.includes('miss')?'cache_misses':'cache_hits'),value,'Acquisition cache metrics for this discovery execution');}saveJson(path.join(root,'outputs/retrieval-metrics.json'),{...acquisition.cache.metrics,observed_at:new Date().toISOString()});
  ...
  const result=await checkpoints.run('evidence',{inputs:['inputs/runtime.json','inputs/reviewed-evidence.json'],execute:async()=>{
   ...
   telemetry.end(packetSpan);telemetry.observe('research.deep_candidates',packets.length,'Reviewed evidence inputs supplied to packets command');
   saveJson(path.join(root,'outputs/evidence-packets.json'),packets);
   const {views,telemetry:contextMetrics}=evidenceViews(packets);for(const [stage,view]of Object.entries(views))saveJson(path.join(root,'outputs',stage+'.json'),view);
   ...
   saveJson(path.join(root,'outputs/context-metrics.json'),contextMetrics);
