...
1. Choose your time above and tap **Add to Google Calendar**. Sign in to your own Google account if asked.
2. Before saving, check the time and time zone, set **Repeats daily** if needed, and set the notification to **0 minutes before**. Mark the event **Free** so it does not block appointments.
...
**Outlook:** Download the file above. In Outlook on the web, choose **Calendar → Add calendar → Upload from file**, select the file and your calendar, then import. In classic Outlook, opening the file may offer an import option. Check that the saved event repeats daily, shows your chosen local time, and has a reminder of **0 minutes**.

**Google Calendar:** Use **Add to Google Calendar**, review the daily repeat and notification settings, then save. Allow calendar notifications in your browser and Windows. Google Calendar must remain open in the browser for its desktop notifications.
...
In your calendar, create an event called **Read the Daily AI Brief**. Choose your preferred start time (9:00 AM is the default suggestion), a five-minute duration, **Repeat every day**, and an alert **at the event time**. Mark it **Free**. Put this link in its description or URL field:
...
The optional shared calendar below uses **9:00 AM local calendar time**. It is read-only and cannot offer a separate time for each subscriber. Use the personal reminder above if you want to choose or edit the time. Some apps suppress alerts for subscribed calendars.
