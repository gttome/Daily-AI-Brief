...
| **T27** | Closure run-state rejects temporary delta-validation receipt | Finalization passed `/tmp/daily-validation.json` into the durable checkpoint contract, which requires repository-relative persisted files | Copy the exact validated receipt into `_records/publication/YYYY-MM-DD/` before run-state finalization and commit it with closure evidence | Closure checkpoint validates only durable repository files; temporary runner paths never enter authoritative run-state |
...
## September 26 durable closure-receipt repair

The exact-SHA post-deployment validation for September 26 passed through live validation and completion construction, then failed only while persisting run-state because a temporary runner file (`/tmp/daily-validation.json`) was supplied as a checkpoint artifact. The run-state contract correctly rejects non-repository files. T27 persists the exact validation receipt under the edition publication record, passes that durable path into finalization, and includes it in the protected completion commit. No reader content, editorial selection, media, Watchlist state, images, protection rules, or live-validation criteria are changed.
